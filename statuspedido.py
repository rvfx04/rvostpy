...
CONVERT(INT, 
    CASE 
        WHEN 
            (CASE 
                WHEN KG_APROB_D / COALESCE(d.KG, 0) > 1 
                THEN 1 
                ELSE KG_APROB_D / COALESCE(d.KG, 0)
            END * cte_produccion.PROG - cte_produccion.CORTADO) < 0
        THEN 0
        ELSE 
            (CASE 
                WHEN KG_APROB_D / COALESCE(d.KG, 0) > 1 
...
if st.sidebar.button("Aplicar filtro"):
    ...
    columns_to_show = ['PEDIDO','F_EMISION', 'F_ENTREGA','DIAS','CLIENTE','PO','KG_REQ','KG_ARMP','KG_TEÑIDOSP','KG_APROB_DP','KG_DESPACHP','UNID','PROGP','CORTADO','CORTADOP','COSIDO', 'COSIDOP','con_tela']
