...
with st.sidebar:
    ...
    if st.button("Aplicar Filtro"):
        ...
        with columns[0]:
            ...
            kgxarm_df = pd.concat([kgxarm_df, totals1_df], ignore_index=True)
            ...
            totals2 = kgxtenir_df.select_dtypes(include=["int", "float"]).sum().rename("Total")
            totals_df = pd.DataFrame(totals2).T
            kgxtenir_df = pd.concat([kgxtenir_df, totals2_df], ignore_index=True)
